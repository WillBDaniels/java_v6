...
public class TestBatchBalance {
	private Utils util = new Utils();
	private DataStore globals;
	private BatchBalanceRequest request;

	public TestBatchBalance(DataStore ds) {
		request = new BatchBalanceRequest();
		globals = ds;
	}

	public BatchBalanceRequest createBatchBalance() {
		BatchBalanceRequest bbr = new BatchBalanceRequest();
		bbr.setDraftLocatorId(globals.getDraftLocatorId());
		bbr.setSystemTraceId(globals.getSystemTraceId());
		bbr.setMerchant(globals.getMerchant());
		bbr.setMerchantRefId(globals.getMerchantRefId());
		bbr.setPaymentInstrumentType(PaymentInstrumentType.CREDIT);
		bbr.setTransactionTimestamp(util.stringToXMLGregorian(globals
