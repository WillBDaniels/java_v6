...
public class TestTokenize {
	...
	private TokenizeRequest request = null;
	private Utils util = new Utils();
	private DataStore globals;

	public TestTokenize(DataStore ds) {
	...
	public void invokeTokenize(PaymentPortType client) {

		TokenizeResponse resp = new TokenizeResponse();
		// com.sun.xml.internal.ws.transport.http.client.HttpTransportPipe.dump
		// = true;

		try {
			resp = client.tokenize(request);
		} catch (ServerFault e) {
			System.out
	...
	public static void main(String[] args) {
		TestTokenize tok = new TestTokenize(new DataStore());
		tok.createTokenizeRequest();
		InitializeClient client = new InitializeClient();
		client.setup();
		tok.invokeTokenize(client.getTestClient());
