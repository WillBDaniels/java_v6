...
public class TestBalanceInquiry {
	...
	private BalanceInquiryRequest request;

	public TestBalanceInquiry(DataStore ds) {
		request = new BalanceInquiryRequest();
		globals = ds;
	}

	public BalanceInquiryRequest createBalanceInquiry() {
		...
		if (globals.getPinEntry().equals("supported"))
			request.setDebit(globals.getDebitInstument());
		else if (!globals.getGiftCardPin().isEmpty())
